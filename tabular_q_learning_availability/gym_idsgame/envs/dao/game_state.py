...
class GameState():
    ...
    def __init__(self, attack_values: np.ndarray = None, defense_values: np.ndarray = None,
        ...
        self.defense_values = defense_values # the defense_values's initial value is defined in   
    ...
    def is_valid_defense(self, target_node_id:int, attack_type: int, defense_node_id: int, defense_type: int , network_config: NetworkConfig) -> bool:
        if network_config.node_list[target_node_id] == NodeType.DATA and \
                network_config.node_list[defense_node_id] == NodeType.DATA and \
                    self.attack_values[target_node_id][attack_type] +1 == self.defense_values[defense_node_id][defense_type]:
            return True 
        else:
            return False
    ...
    def defend(self, node_id: int, defense_type: int, max_value: int, network_config: NetworkConfig,
        ...
        """
        if defense_type==-1 or (defense_type % self.defense_values.shape[1]+1) == 0 and defense_type>0:
            return False
    ...
    def simulate_detection(self, node_id: int, reconnaissance: bool, reconnaissance_detection_factor : float = 1) -> bool:
        ...
        if not reconnaissance:
            return np.random.rand() < self.defense_det[node_id]/20
        else:
            det_prob = (self.defense_det[node_id] / 20)*reconnaissance_detection_factor
