...
def default_config() -> ClientConfig:
    ...
    """
    q_agent_config = QAgentConfig(gamma=0.999, alpha=0.0005, epsilon=0.5, render=False, eval_sleep=0.9,
                                  min_epsilon=0.01, eval_episodes=100, train_log_frequency=100,
                                  epsilon_decay=0.9999, video=False, eval_log_frequency=1,
                                  video_fps=5, video_dir=default_output_dir() + "/results/videos", num_episodes=20001,
                                  eval_render=False, gifs=False, gif_dir=default_output_dir() + "/results/gifs",
...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("maximal_attack_vs_tabular_q_learning-v0", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
    print(config.q_agent_config)
